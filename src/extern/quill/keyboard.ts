...
class Keyboard extends Module<KeyboardOptions> {
	...
	listen() {
		this.quill.root.addEventListener("keydown", (evt) => {
			...
			const blot = Quill.find(evt.target as any, true);
					...
					if (
						!Object.keys(binding.format).every((name) => {
							...
							if (binding.format[name] === false)
								return curContext.format[name] == null;
							// @ts-expect-error Fix me later
							return deepCompare(binding.format[name], curContext.format[name]);
						})
					) {
						return false;
					}
...
const defaultOptions: KeyboardOptions = {
	bindings: {
		...
		"table enter": {
			...
			format: ["table"],
			handler(range) {
				const module: any = this.quill.getModule("table");
				if (module) {
...
function makeTableArrowHandler(up: boolean): BindingObject {
	return {
		...
		handler(_range, context) {
			...
			const cell: any = context.line;
			const targetRow = cell.parent[key] as any;
			if (targetRow != null) {
...
function normalize(binding: any): BindingObject | null {
